...
export default function AlertPanel({ alerts = [] }) {
    ...
    const getLabel = (alert) => {
        const confidence = alert.confidence || 0;
        const frames = alert.frames_detected ? ` (${alert.frames_detected} frames)` : '';
        
        if (alert.prediction === "Violence" || alert.type === "violence") {
            return `🔴 VIOLENCE DETECTED${frames} - ${(confidence * 100).toFixed(1)}%`;
        }
                ...
                {alerts.slice(0, 10).map((alert, index) => (
                    ...
                    >
                        ...
                        <div className="mt-1 text-sm">
                            Confidence: <b>{((alert.confidence || 0) * 100).toFixed(1)}%</b>
                        </div>
                        {alert.frames_detected && (
                            <div className="text-xs opacity-60 mt-1">
                                Frames: {alert.frames_detected}
                            </div>
                        )}
                    </div>
                ))}
